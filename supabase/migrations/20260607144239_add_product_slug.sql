...
DECLARE
    duplicate RECORD;
    dup_row RECORD; -- FIXED: Added variable declaration
    counter INTEGER;
BEGIN
    FOR duplicate IN 
        ...
        HAVING COUNT(*) > 1
    LOOP
        counter := 0; -- Starts at 0 so the first item stays original, and subsequent ones get numbered
        FOR dup_row IN 
...
-- Step 4: Add unique constraint (Safely drops old one if it exists first)
